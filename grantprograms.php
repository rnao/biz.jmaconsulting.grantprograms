...
function grantprograms_civicrm_pre($op, $objectName, $id, &$params) {
  if ($objectName == 'Grant' && ($op == 'edit' || $op == 'create')) {
    ...
    $session->createScope('civigrant');

    $grantParams = $params; // save what we know so far; varies on whether edit or create
    ...
    if (isset($params['restrictEmail']) && $params['restrictEmail']) {
      $session->set('restrictGrantEmail', true, 'civigrant');
    }

    if (!isset($grantParams['grant_type_id'])) {
      $grant = CRM_Grant_BAO_Grant::retrieve($grantParams, $emptyArray);
      $params['grant_type_id'] = $grant->grant_type_id;
    }

    // Do we need the grant ID in there really?
    if ($op == 'create') {
      $session->set("grant_new_params", $params, 'civigrant');
    } else {
      $session->set("grant_" . $id . "_params", $params, 'civigrant');
    }
...
function grantprograms_civicrm_post($op, $objectName, $objectId, &$objectRef) {
  ...
  // Send email after grant save
  if ($objectName == 'Grant') {
    $session = CRM_Core_Session::singleton();
    $restrictEmail = $session->get('restrictGrantEmail', 'civigrant');

    // Manual override?
    if ($restrictEmail === null) {
      if ($op == 'create') {
        $params = $session->get('grant_new_params', 'civigrant');
      } else {
        $params = $session->get('grant_' . $objectId . '_params', 'civigrant');
      }
      ...
      if (isset($endDate)) {
        ...
        if (($newDate <= date('Y-m-d') || date('Y') < date('Y',strtotime($endDate))) &&
          $params['status_id'] == $grantStatus['Submitted'])
        {
          $reasonGrantIneligible = CRM_Core_OptionGroup::values('reason_grant_ineligible');
          $reasonGrantIneligible = array_flip($reasonGrantIneligible);
          $params['status_id'] = $grantStatus['Ineligible'];
          $params['grant_rejected_reason_id'] = $reasonGrantIneligible['Outside dates'];
  ...
  if ($objectName == 'Grant' && ($op == 'edit' || $op == 'create') && $objectRef->financial_type_id) {
    ...
    if ($objectRef->status_id == array_search('Approved for Payment', $status)) {
      $params['to_financial_account_id'] = CRM_Contribute_PseudoConstant::getRelationalFinancialAccount($objectRef->financial_type_id, 'Accounts Payable Account is');
...
ORDER BY cft.id DESC LIMIT 1");
    ...
    if (CRM_Utils_Array::value('to_financial_account_id', $params)) {
      ...
      if ($previousGrant && $previousGrant->status_id == array_search('Approved for Payment', $status) && $objectRef->status_id == array_search('Paid', $status)) {
        $trxnParams['from_financial_account_id'] = CRM_Contribute_PseudoConstant::getRelationalFinancialAccount($objectRef->financial_type_id, 'Accounts Payable Account is');
      }
      $trxnId = CRM_Core_BAO_FinancialTrxn::create($trxnParams, $trxnEntityTable);
      if ($createItem) {
        $financialAccountId = CRM_Contribute_PseudoConstant::getRelationalFinancialAccount($objectRef->financial_type_id, 'Expense Account is');
        $itemParams = array(
